      ...
      if (res?.body && typeof res.body.getReader === 'function') {
        ...
        const readable = new ReadableStream({
          async start(controller) {
            ...
            /* ---------- FINALIZE & SAVE ---------- */
            if (body.method === 'agent/run') {
              const finalized = finalizeRunEvents(events)

              const messagesToSave = finalized.messages.map((msg) => ({
                id: msg.id,
                role: msg.role,
                content: msg.content,
                toolCalls: msg.toolCalls,
              ...
              if (messagesToSave.length > 0) {
                ...
                /* ---------- Incremental context ---------- */
                const last5 = messagesToSave.slice(-5)
                const previousContext = await loadContext(threadId).catch(
                  () => null
                )
